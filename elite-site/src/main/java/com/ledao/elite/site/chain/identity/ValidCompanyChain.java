 ...
 * @version 1.0
 * */
@Component("validCompany")
public class ValidCompanyChain extends HandlerChain{
	...
	public void handle(Object args) throws EliteBusinessException {
		...
		for(String key:identityMap.keySet()){
			if(MemberIdentity_Type.company.name().equals(key)){
				flag=true;
			}
		}
		if(!flag){
			throw new UnauthorizedOfException("没有项目方的权限，不能执行操作");
		}
	}
