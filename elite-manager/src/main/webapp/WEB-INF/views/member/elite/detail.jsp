...
<body class="skin-blue">
										...
										<tr>
											<td>
												<div style="padding: 5px 10px; text-align: left;">
													...
													<div>
														<p>
															<span>注册时间 ：<fmt:formatDate value="${member.createTime}" pattern="yyyy-MM-dd HH:mm" /></span> <span style="margin-left: 20px;">编号:${member.memberNum}</span> <span
																style="text-align: right; margin-left: 50px;">${member.basic.areaName}</span>
														</p>
														...
														<div class="project-info">
															...
															<div style="margin-bottom: 6px; text-align: right; padding-top: 15px;">
																...
																<div style="margin-bottom: 6px; text-align: right;">
																	<span style="text-align: right;" class="index">诚信度: <span style="color: #000; text-align: right;">${member.basic.integrity}</span>
																	</span>
																</div>
																<div style="margin-bottom: 6px; text-align: right;"></div>
															</div>
														</div>
													</div>
												</div>
											...
											<td>
												<div style="padding-top: 15px;">
													<div style="margin-bottom: 6px; text-align: gray;">我的收益${member.memberAccount.totalIncome}元</div>
													<!-- <div style="margin-bottom: 6px; text-align: gray;">已接任务数 &nbsp;</div> -->
												</div>
											</td>
											<td>
												<div style="margin-top: 25px;" class="opt">
													<shiro:hasPermission name="elite:member:elite:aduit">
														<c:choose>
															<c:when test="${member.elite.status == 'waitAduit'}">
																<p>
																	<a style="margin-bottom: 6px;" href="javascript:void(0);" id="audit" data="${member.id}">审核</a> <input type="button"
																		style="background-color: red; height: 12px; width: 12px; border: 1px; border-radius: 6px;"></input>
																</p>
															</c:when>
														</c:choose>
													</shiro:hasPermission>
													<shiro:hasPermission name="elite:member:elite:viewLevel">
														<p>
															<a style="margin-bottom: 6px;" href="javascript:void(0);" id="level" data="${member.id}">重新定级</a>
														</p>
													</shiro:hasPermission>
												</div>
											</td>
					...
					<div>
						...
						<div id="myTabContent" class="tab-content">
							...
							<div id="tab_1" class="tab-pane fade in active">
								...
								<div style="margin-left: 2px; list-style-type: none;" class="tab-pane fade in active" id="home">
								...
								<li style="list-style: none; margin-top: 5px; margin-bottom: 6px; clear: left;"><label> <c:if test="${member.elite.ctoSigned=='true'}">
					申请
									</c:if> <c:if test="${member.elite.ctoSigned=='false'}">
					未申请参与 
									</c:if>
								</label> <a href="#heh">《云英汇CTO签约计划》</a></li>
								...
								<div>
									生日年月：
									<fmt:formatDate value="${member.basic.birthday}" pattern="yyyy-MM-dd" />
								</div>
								...
								<div style="margin-bottom: 6px;" class="common-red">
									...
									<c:forEach items="${member.elite.getAttentionIndustryListVal()}" var="eej">
										${eej}
								</c:forEach>
									<span></span>
								</div>
								...
								<br> <label class="common-red">教育经历<span>*</span></label>
								...
								<br> <label style="margin-bottom: 6px;" class="common-red">附件简历<span>*</span></label>
								<div>
									<c:if test="${member.elite.resumeAttaId!=null }">
									${member.elite.resumeAtta.fileName} <a href="${member.elite.resumeAtta.downPath}" target="_blank"> <span class="glyphicon glyphicon-download custom-icon"></span>
										</a>
									</c:if>
								</div>
							</div>
							...
							<div id="tab_2" class="tab-pane fade ">
								<div>
									...
									<div class="tab1-item">
										<label class="title-style2">真实姓名:</label> <label class="detail-style2">${member.credit.realName}</label>
									</div>
									<div class="tab1-item">
	...
	<script src="${_PATH}/res/js/admin/member/elite/detail.js"></script>
	<script src="${_PATH}/res/js/admin/member/elite/audit.js"></script>
