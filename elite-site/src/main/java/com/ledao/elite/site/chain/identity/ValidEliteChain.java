 ...
 * @version 1.0
 * */
@Component("validElite")
public class ValidEliteChain extends HandlerChain{
	...
	public void handle(Object args) throws EliteBusinessException {
		...
		for(String key:identityMap.keySet()){
			if(MemberIdentity_Type.elite.name().equals(key)
					||MemberIdentity_Type.cto.name().equals(key)){
				flag=true;
			}
		}
		if(!flag){
			throw new UnauthorizedOfException("没有精英的权限，不能执行操作");
		}
	}
