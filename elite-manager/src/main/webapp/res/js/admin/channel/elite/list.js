...
var list = {
	...
	search : function() {
		jsonAjax.post({
			url : ctx + '/channel/elite/listData',
			...
			success : function(data) {
				...
				$("a[id=workLogs]").click(list.showWorkLogs);
	...
	auditAgree : function() {
		$('#auditstatus').val("normal");
		list.auditSubmit();
	},
	auditReject : function() {
		$('#auditstatus').val("auditNotPass");
		list.auditSubmit();
	},
	auditSubmit : function() {
		Ajax.post({
			url : ctx + '/channel/elite/audit',
	...
	detail : function() {
		list.initSetSearchData();
		  var id = $(this).attr("data");
			...
			$('#detailForm').attr('action', ctx + "/channel/elite/detail").submit();
	},
	allotPm : function() {
	    var id = $(this).attr("data");
		$("#detailFormInputId").val(id);
	...
	initGetSearchData:function(){
		var keyword=sessionStorage.getItem('partnerElitekeyword');
		if(keyword!=null){
			$("#keywordId").val(keyword);
			sessionStorage.removeItem("partnerElitekeyword");
		}
		var status=sessionStorage.getItem('partnerElitestatus');
		if(status!=null){
			$("#partnerStatus").val(status);
			sessionStorage.removeItem("partnerElitestatus");
		}
		var searchRole=sessionStorage.getItem('partnerEliteSearchRole');
		if(searchRole!=null){
			$("#searchRole").val(searchRole);
			sessionStorage.removeItem("partnerEliteSearchRole");
		}
		var pageth=sessionStorage.getItem('partnerElitepageth');
		if(pageth!=null)
		{
			$("#initPageth").html("<input type='hidden' name='pageth'  value='"+pageth+"'/>")
			sessionStorage.removeItem('partnerElitepageth');
		}
	...
	initSetSearchData:function(){
		...
		var searchRole = $("#searchRole").find("option:selected").val();
		var pageth = $("#pager_pageth").val();
		if (keyword != null) {
			sessionStorage.setItem('partnerElitekeyword', keyword);
		}
		if(status!=null){
			sessionStorage.setItem('partnerElitestatus', status);
		}
		if(pageth!=null)
		{
			sessionStorage.setItem('partnerElitepageth', pageth);
		}
		if(searchRole!=null)
		{
			sessionStorage.setItem('partnerEliteSearchRole', searchRole);
		}
