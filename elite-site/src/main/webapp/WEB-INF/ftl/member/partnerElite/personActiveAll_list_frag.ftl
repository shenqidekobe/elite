<div class="person-table table" id="table1">
    ...
    <#list list as it>
	    <div class="groupBox clearfloat">
	    	...
	    	<span class="part2 part">
              ${it.memberPassport.nickName}
             </span>
	    	<span class="part3 part">
	    	<#if it.memberCredit.realName??>
	    	 ${it.memberCredit.realName}
	    	<#else>
	    	 ${it.name}
	    	</#if>
            </span>
	    	<span class="part4 part">${it.phone}</span>
	    	<span class="part5 part">${it.status.label}</span>
	    	<span class="part6 part">${it.tasksInCount}</span>
	    	<span class="part7 part">
	    	    <#if it.memberId??>
                  ${it.memberPassport.lastLoginTime?string("yyyy.MM.dd")}
               </#if></span>
	    	<span class="part8 part">
             <#if it.lastRecruitTime??>
                           ${it.lastRecruitTime?string("yyyy.MM.dd")}
              <#else>
                                                  未曾接单
               </#if></td>
