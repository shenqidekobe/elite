...
<script>
 function back(){
	 window.location.href = ctx + '/settlement/platformOutOrder'
 }
</script>
<head>
					...
					<div class="box">
						...
						<div class="box-body" style="text-align: left;">
							<div id="listData">
                                  ...
                                  <div>会员的收款帐号：${it.memberAccount}</div>
                                  <div>订单状态：${it.status.label}</div>
                                  <div>订单类型：${it.type.label}</div>
                                  <div>订单金额：${it.orderAmount}</div>
                                  <div>税款金额：${it.taxAmount}</div>
                                  <div>实付金额：${it.receiptAmount}</div>
                                <!--   <div>积分金额：</div>
                                  <div>优惠券金额：</div> -->
                                  <div>备注：${it.intro }</div>
                                  <div>质保金支付时间：<fmt:formatDate value="${it.payqualityTime}" pattern="yyyy-MM-dd HH:mm:ss" /></div>
                                  <div>创建时间：<fmt:formatDate value="${it.createTime}" pattern="yyyy-MM-dd HH:mm:ss" /></div>
							</div>
						</div>
					</div>
