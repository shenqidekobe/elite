	...
	<tbody id="tbody">
		<c:forEach items="${list}" var="it">
			<tr style="text-align: center;">
				<td colspan="2"><small>注册时间:<fmt:formatDate value="${it.partnerCompany.createTime}" pattern="yyyy-MM-dd HH:mm" /></small></td>
				<td colspan="2"><small>渠道编号:${it.partnerCompany.channelNum}</small></td>
				<td><small>Ta的推荐人:${it.partnerCompany.parentName}</small></td>
				<td><small>Ta的负责人:${it.partnerCompany.chargeName}</small></td>
			</tr>
			<tr>
				...
				<td>
					...
					<div>机构:${it.partnerCompany.companyName}</div>
					<div>类别:${it.partnerCompany.type.label}</div>
				</td>
				<td>
					<div>收益总额:${it.memberAccount.totalIncome}</div>
					<div>备案及入驻项目数:${it.partnerCompany.putCount}(${it.partnerCompany.enterCount})</div>
					<div>备案及入驻项目渠道数:${it.partnerCompany.putParnterCount}(${it.partnerCompany.enterParnterCount})</div>
				</td>
				<td>
				　　	<div>
						<div>
							...
							<c:if test="${it.status!='disabled'}">
								${it.partnerCompany.status.label}
							</c:if>
						</div>
				...
				<td>
					<div style="margin-top: 25px;" class="opt">
						<div>
							...
							<shiro:hasPermission name="elite:channel:project:aduit">
								<c:if test="${it.partnerCompany.status=='aduitIn'}">
							...
							<shiro:hasPermission name="elite:channel:project:remark">
