...
<%@ include file="/inc/taglibs.jsp"%>
  <h5 class="box-title" align="left">审核通过精英( ${normalCount} )&nbsp;&nbsp; 共注册精英( ${allCount} )</h5>
			...
			<tr>
						...
						<div>
							...
							<div class="project-info">
								...
								<c:forEach items="${it.elite.eliteJobs}" var="eej">
									<div style="margin-bottom: 6px;width:110px" class="title-label">
										${eej.getJobRoleVal()}
								...
								<div style="margin-bottom: 6px; text-align: right;">
								       <small> <c:if test="${it.inviteCode!=null}">
								 	 邀请码：${it.inviteCode}
									</c:if></small> 
								...
								<div
									style="margin-bottom: 6px; text-align: right; padding-top: 0px;">
									<div>
				...
				<td>
					<div style="margin-top: 15px;">
					   ...
					   <div style="margin-bottom: 6px;">${it.elite.status.label}</div>
						...
						<shiro:hasPermission name="elite:member:elite:viewDetail">
						<p style="margin-bottom: 6px;">
							<a href="javascript:void(0);" id="detail" data="${it.id}">查看详情
							</a>
						</p>
						</shiro:hasPermission>
					</div>
				...
				<td>
					<div style="padding-top: 15px;">
						<div style="margin-bottom: 6px; text-align: gray;">我的收益${it.memberAccount.totalIncome}
							元</div>
					</div>
				</td>
				<td>
					<div style="margin-top: 25px;" class="opt">
						...
						<div>
							...
							<p>
								<a style="margin-bottom: 6px; margin-top: 2px;"
									href="javascript:void(0);" id="level" data="${it.id}">重新定级
								</a>
							</p>
							</shiro:hasPermission>
							<shiro:hasPermission name="elite:member:elite:homeShow">
							<c:if test="${it.elite.status=='normal'}">
							...
							<shiro:hasPermission name="elite:member:elite:enable">
							<c:if test="${it.status=='disabled'}">
								<p>
									<a style="margin-bottom: 6px;" href="javascript:void(0);"
							...
							<c:choose>
								<c:when test="${it.elite.status == 'waitAduit'}">
									<p>
										<a style="margin-bottom: 6px;" href="javascript:void(0);"
											id="audit" data="${it.id}">强审核</a> <input type="button"
											style="background-color: red; height: 12px; width: 12px; border: 1px; border-radius: 6px;"></input>
									</p>
								</c:when>
								<c:when test="${it.elite.status == 'normal'}">
									<c:if test="${it.elite.ctoSigned=='true'}">
										<p>
											<a style="margin-bottom: 6px;" href="javascript:void(0);"
												id="audit" data="${it.id}">审核成为CTO</a> <input type="button"
												style="background-color: red; height: 12px; width: 12px; border: 1px; border-radius: 6px;"></input>
										</p>
									</c:if>
								</c:when>
							</c:choose>
							<c:if test="${it.elite.status!='waitAduit'&&it.elite.status!='aduitIn'}">
								<p>
									<a style="margin-bottom: 6px;" href="javascript:void(0);"
										id="audit" data="${it.id}">强审核</a>
								</p>
							</c:if>
							<c:if test="${it.elite.status=='aduitIn'}">
								<p>
									<a style="margin-bottom: 6px;" href="javascript:void(0);"
								...
								<shiro:hasPermission name="elite:member:elite:disable">
								<p>
								   <a style="margin-bottom: 6px;" href="javascript:void(0);" id="disabledBtn" data="${it.id}">禁用</a>
								</p>
								</shiro:hasPermission>
							</c:if>
						</div>
